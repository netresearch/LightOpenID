...
class LightOpenID
{
    ...
    /**
     * @var string[] Required SREG parameters
     */
    public $required = array();

    /**
     * @var string[] Optional SREG parameters
     */
    public $optional = array();

    /**
     * @var boolean CURLOPT_SSL_VERIFYPEER
     */
    public $verify_peer = null;

    /**
     * @var string CURLOPT_CAPATH
     */
    public $capath = null;

    /**
     * @var string CURLOPT_CAINFO
     */
    public $cainfo = null;

    /**
     * @var string SSL peer name
     */
    public $cnmatch = null;
    ...
    /**
     * @var int curl overall timeout in seconds
     */
    ...
    /**
     * @var bool Flag OpenID Attribute Excahnge format used
     */
    protected $ax = false;

    /**
     * @var bool Flag OpenID Simple Registration Extension format used
     */
    ...
    function __construct($host, $proxy = null)
    {
        ...
        $uri = preg_replace(
            '#((?<=\?)|&)openid\.[^&]+#',
            '',
            (isset($uri['path']) ? $uri['path'] : '') . (isset($uri['query']) ? '?' . $uri['query'] : '')
        );
    ...
    /**
     * @param array|string $proxy Proxy setup
     *
     * @throws ErrorException
     */
    ...
    {
        if (empty($proxy)) {
            throw new \InvalidArgumentException("'proxy' must be an array or URL.");
        }
     ...
     * @return string
     */
    protected function build_url(array $url, array $parts)
    {
    ...
    /**
     * Returns value of given attribute in given HTML meta or link tag.
     *
     ...
     * @return string
     */
    protected function getHtmlTagAttributeValue($content, $tag, $attrName, $attrValue, $valueName)
    {
        preg_match_all(
    ...
    /**
     * @deprecated by self::getHtmlTagAttributeValue()
     */
    protected function htmlTag($content, $tag, $attrName, $attrValue, $valueName)
    {
        return $this->getHtmlTagAttributeValue($content, $tag, $attrName, $attrValue, $valueName);
    ...
    function discover($url)
    {
        ...
        for ($i = 0; $i < 5; $i ++) {
            if ($yadis) {
                ...
                $location = $this->getHtmlTagAttributeValue($content, 'meta', 'http-equiv', 'X-XRDS-Location', 'content');
                if ($location) {
            ...
            $server   = $this->getHtmlTagAttributeValue(
                $content, 'link', 'rel', 'openid2.provider', 'href'
            );
            $delegate = $this->getHtmlTagAttributeValue(
                $content, 'link', 'rel', 'openid2.local_id', 'href'
            );
            $this->version = 2;
            ...
            if (!$server) {
                ...
                $server   = $this->getHtmlTagAttributeValue(
                    $content, 'link', 'rel', 'openid.server', 'href'
                );
                $delegate = $this->getHtmlTagAttributeValue(
                    $content, 'link', 'rel', 'openid.delegate', 'href'
                );
                $this->version = 1;
            }
    ...
    /**
     * @deprecated by self::getSregParams()
     */
    protected function sregParams()
    {
        return $this->getSregParams();
    }

    /**
     * @return array OpenID Simple Registration Extension parameters
     */
    protected function getSregParams()
    {
        $params = array();
    ...
    /**
     * @deprecated by self::getAxParams()
     */
    protected function axParams()
    {
        return $this->getAxParams();
    }

    /**
     * @return array OpenID Attribute Exchange parameters
     */
    protected function getAxParams()
    {
        $params = array();
    ...
    /**
     * Returns OpenID v1 authentication URL.
     *
     * @param boolean $immediate Enable OpenID mode checkid_immediate
     *
     * @return string
     */
    ...
    /**
     * Returns OpenID v2 authentication URL.
     *
     * @param boolean $immediate Enable OpenID mode checkid_immediate
     *
     * @return string
    ...
    protected function authUrl_v2($immediate)
    {
        ...
        if ($this->ax) {
            $params += $this->getAxParams();
        }

        if ($this->sreg) {
            $params += $this->getSregParams();
        }

        if (!$this->ax && !$this->sreg) {
            # If OP doesn't advertise either SREG, nor AX, let's send them both
            # in worst case we don't get anything in return.
            $params += $this->getAxParams() + $this->getSregParams();
        }
     ...
     *
     * @param boolean $immediate Enable OpenId mode immediate
     *
    ...
    function validate()
    {
        ...
        $this->claimed_id = isset($this->data['openid_claimed_id'])
    ...
    /**
     * Returns OpenID Attribute Exchange attributes.
     *
     * @return array
     */
    ...
    /**
     * Returns OpenID Simple Registration Extension attributes.
     *
     * @return array
     */
